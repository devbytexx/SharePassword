...
// --- counters (datenschutzkonforme Nutzungszähler, nur Integer) ---

const COUNTER_NAMES = new Set(['created', 'viewed']);

export async function incrementCounter(name) {
  if (!COUNTER_NAMES.has(name)) return;   // Whitelist: keine beliebigen Keys
...
export async function getCounters() {
  const [rows] = await getPool().execute(
    `SELECT name, value FROM counters WHERE name IN ('created', 'viewed')`
  );
  const out = { created: 0, viewed: 0 };
